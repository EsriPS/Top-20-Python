...
class MashUp:
    ...
    def merge_results(self, lists_of_dicts: list[list[dict]]):
        """
        Merge multiple lists of results dictionaries into a single list of unique
        dictionaries, filtering out duplicates with the same Portal URL and id,
        keeping the one with the latest modified date.
        ...
        for dictionary in merged_lists:
            # Use a tuple of the id and portal URL as the key
            key = (dictionary["id"], dictionary["portal_url"])
            # If the dictionary is not in the unique_dicts or the modified date is greater
            if (
    ...
    def search(self, start=0, limit=None):
        ...
        for portal in self.portals:
            if portal.url in self._last_result_date:
                # This string is used to search for items modified after the last result date
            ...
            if modified_string and self.append_search_string:
                # If we have a modified string and an append search string, we need to combine them
                search_string = (
                    f"({modified_string}) AND ({self.append_search_string}))"
                )
            elif modified_string:
                # If we only have a modified string, we use that
                search_string = modified_string
            elif self.append_search_string:
                # If we only have an append search string, we use that
                search_string = self.append_search_string
            else:
                # If we have neither, we use None
                search_string = None
            ...
            if results["results"]:
                # Store the last modified date for the portal
